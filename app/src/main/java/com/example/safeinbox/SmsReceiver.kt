                ...
                TurboExecutor.getInstance().execute {
                    try {
                        ...
                        for (message in messages) {
                            try {
                                val rawSender = message.displayOriginatingAddress ?: "Unknown"
                                val sender = ContactUtils.normalizeSender(rawSender)
                                ...
                                val contactName = ContactUtils.getContactName(context, rawSender)
                                ...
                                val isSpam = (result.status == com.example.safeinbox.models.ClassificationResult.Status.SPAM || 
                                              result.status == com.example.safeinbox.models.ClassificationResult.Status.SUSPICIOUS)
