...
public class SpamDao {
    ...
    private static boolean isRescanMode = false;
    ...
    private void incrementSenderScoreInTransaction(SQLiteDatabase db, String sender, boolean isSpam) {
        ...
        if (isRescanMode) {
            // Turbo: Update cache directly to avoid O(N) DB hits in rescan loop
            SenderReputation rep = reputationCache.get(normalized);
            if (rep == null) rep = new SenderReputation();
            if (isSpam) rep.spamHits++; else rep.hamHits++;
            reputationCache.put(normalized, rep);
        } else {
            // Real-time: Evict to ensure fresh data
    ...
    public void updateMessageStatus(long messageId, String status, boolean isSpam, int score, String reason) {
        try {
            SQLiteDatabase db = dbHelper.getWritableDatabase();
            ContentValues v = new ContentValues();
            v.put(Constants.COL_IS_SPAM, isSpam ? 1 : 0);
            v.put(Constants.COL_CLASSIFICATION_STATUS, status);
            v.put(Constants.COL_SCORE, score);
            v.put(Constants.COL_REASON, reason);
            db.update(Constants.TABLE_MESSAGES, v, Constants.COL_ID + " = ?", new String[]{String.valueOf(messageId)});
        } catch (Exception e) {
            Log.e("SpamDao", "updateMessageStatus error", e);
        }
    }
    ...
    public long insertMessage(SmsMessage message) {
        try {
            ...
            boolean isSpamDetected = "SPAM".equals(message.getClassificationStatus()) || "SUSPICIOUS".equals(message.getClassificationStatus());
            ...
            values.put(Constants.COL_CLASSIFICATION_STATUS, message.getClassificationStatus());
            values.put(Constants.COL_HAS_RISKY_LINK, message.isHasRiskyLink() ? 1 : 0);
            values.put(Constants.COL_SCORE, message.getScore());
            values.put(Constants.COL_REASON, message.getClassificationReason());
    ...
    public void insertMessagesBatch(List<SmsMessage> messages) {
        ...
        try {
            for (SmsMessage message : messages) {
                ...
                boolean isSpam = isBlocked || "SPAM".equals(status) || "SUSPICIOUS".equals(status);
                
                values.put(Constants.COL_IS_SPAM, isSpam ? 1 : 0);
                values.put(Constants.COL_IS_BLOCKED, isBlocked ? 1 : 0);
                
                values.put(Constants.COL_DEDUP_ID, dedupId);
                values.put(Constants.COL_CLASSIFICATION_STATUS, status);
                values.put(Constants.COL_HAS_RISKY_LINK, message.isHasRiskyLink() ? 1 : 0);
                values.put(Constants.COL_SCORE, message.getScore());
                values.put(Constants.COL_REASON, message.getClassificationReason());
    ...
    public List<SmsMessage> getMessages(boolean spamOnly) {
        ...
        try {
            ...
            if (spamOnly) {
                // SPAM Filter: Only messages definitively marked as SPAM, SUSPICIOUS or globally blocked
                query = "SELECT * FROM " + Constants.TABLE_MESSAGES
                        + " WHERE (" + Constants.COL_CLASSIFICATION_STATUS + " IN ('SPAM', 'SUSPICIOUS')"
                        + " OR (" + Constants.COL_CLASSIFICATION_STATUS + " IS NULL AND " + Constants.COL_IS_SPAM + " = 1))"
                        + " ORDER BY " + Constants.COL_DATE + " DESC LIMIT 1000";
                args = null;
            } else {
                // INBOX Filter: messages marked as HAM, SAFE, OTP, or SERVICE
                query = "SELECT * FROM " + Constants.TABLE_MESSAGES
                        + " WHERE (" + Constants.COL_CLASSIFICATION_STATUS + " IN ('HAM', 'SAFE', 'OTP', 'SERVICE')"
            ...
            if (cursor != null) {
                try {
                    if (cursor.moveToFirst()) {
                        ...
                        msg.setScore(cursor.getInt(cursor.getColumnIndexOrThrow(Constants.COL_SCORE)));
                        msg.setClassificationReason(cursor.getString(cursor.getColumnIndexOrThrow(Constants.COL_REASON)));
                        return msg;
                    }
                } finally {
                    cursor.close();
    ...
    public void insertSimilaritySample(String body, String label) {
        try {
            SQLiteDatabase db = dbHelper.getWritableDatabase();
            ContentValues values = new ContentValues();
            values.put(Constants.COL_BODY_HASH, String.valueOf(body.hashCode()));
            values.put(Constants.COL_BODY, body);
            values.put(Constants.COL_LABEL, label);
            db.insertWithOnConflict(Constants.TABLE_SIMILARITY_DATA, null, values, SQLiteDatabase.CONFLICT_REPLACE);
        } catch (Exception e) {
            Log.e(TAG, "insertSimilaritySample error", e);
        }
    }

    public List<SmsMessage> getSimilaritySamples() {
        List<SmsMessage> samples = new ArrayList<>();
        try {
            SQLiteDatabase db = dbHelper.getReadableDatabase();
            Cursor cursor = db.query(Constants.TABLE_SIMILARITY_DATA, null, null, null, null, null, null);
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    SmsMessage msg = new SmsMessage();
                    msg.setBody(cursor.getString(cursor.getColumnIndexOrThrow(Constants.COL_BODY)));
                    msg.setClassificationStatus(cursor.getString(cursor.getColumnIndexOrThrow(Constants.COL_LABEL)));
                    samples.add(msg);
                }
                cursor.close();
            }
        } catch (Exception e) {
            Log.e(TAG, "getSimilaritySamples error", e);
        }
        return samples;
    }

    public void reclassifySimilarMessages(String triggerBody, boolean isSpam) {
        try {
            // Find messages with visually similar bodies (same alphanumeric fingerprint)
            String fingerprint = normalizeBody(triggerBody);
            SQLiteDatabase db = dbHelper.getWritableDatabase();
            
            ContentValues values = new ContentValues();
            values.put(Constants.COL_IS_SPAM, isSpam ? 1 : 0);
            values.put(Constants.COL_CLASSIFICATION_STATUS, isSpam ? "SPAM" : "HAM");
            
            // Forensic re-evaluation: We update ALL messages that share this core fingerprint
            int rows = db.update(Constants.TABLE_MESSAGES, values, 
                    Constants.COL_DEDUP_ID + " LIKE ?", 
                    new String[]{"%|" + fingerprint + "|%"});
                    
            Log.d(TAG, "Reclassified " + rows + " similar messages to " + (isSpam ? "SPAM" : "HAM"));
            
            // Store the seed in the similarity dataset for Layer 3 detection
            insertSimilaritySample(triggerBody, isSpam ? "SPAM" : "HAM");
            
        } catch (Exception e) {
            Log.e(TAG, "reclassifySimilarMessages error", e);
        }
    }
    ...
    public void updateSpamStatus(long messageId, boolean isSpam) {
        try {
            ...
            // Real-time Reclassification: Force re-run on similar stored messages
            reclassifySimilarMessages(msg.getBody(), isSpam);
    ...
    public void removeSpamNumber(String number) {
        ...
        try {
            ...
            // 2. Update ALL messages from this sender: clear blocked flag AND reset to HAM
            ContentValues values = new ContentValues();
            values.put(Constants.COL_IS_SPAM, 0);
            values.put(Constants.COL_IS_BLOCKED, 0);
            values.put(Constants.COL_CLASSIFICATION_STATUS, "HAM");
            ...
            Log.d(TAG, "Unblocked sender and restored ALL messages to HAM: " + normalizedNumber);
        } catch (Exception e) {
    ...
    public int getRecentMessageCount(String sender, long windowMillis) {
        if (isRescanMode) return 0; // Optimization: Skip behavioral DB hits during rescan
        int count = 0;
        long since = System.currentTimeMillis() - windowMillis;
        try (Cursor cursor = dbHelper.getReadableDatabase().rawQuery(
                "SELECT COUNT(*) FROM " + Constants.TABLE_MESSAGES + " WHERE " + Constants.COL_SENDER + " = ? AND " + Constants.COL_DATE + " > ?",
                new String[]{sender, String.valueOf(since)})) {
            if (cursor.moveToFirst()) count = cursor.getInt(0);
        } catch (Exception e) {
            Log.e(TAG, "getRecentMessageCount error", e);
        }
        return count;
    }

    /**
     * ADVANCED: Tracks how many times a SIMILAR message pattern has appeared from this sender.
     * Uses the body fingerprint to find 'Nagging' campaigns.
     */
    public int getRecentSimilarMessageCount(String sender, String body, long windowMillis) {
        if (isRescanMode) return 0; // Optimization: Skip expensive LIKE queries during rescan
        if (body == null || body.length() < 10) return 0; // Ignore short/generic messages
        int count = 0;
        long since = System.currentTimeMillis() - windowMillis;
        String fingerprint = generateBodyFingerprint(new SmsMessage(sender, body, 0));
        
        try (Cursor cursor = dbHelper.getReadableDatabase().rawQuery(
                "SELECT COUNT(*) FROM " + Constants.TABLE_MESSAGES + 
                " WHERE " + Constants.COL_SENDER + " = ? " +
                " AND " + Constants.COL_DATE + " > ? " +
                " AND " + Constants.COL_DEDUP_ID + " LIKE ?",
                new String[]{sender, String.valueOf(since), "%" + fingerprint + "%"})) {
            if (cursor.moveToFirst()) count = cursor.getInt(0);
        } catch (Exception e) {
            Log.e(TAG, "getRecentSimilarMessageCount error", e);
        }
        return count;
    }
    ...
    /**
     * INDUSTRIAL SYSTEM INGESTION:
     * Scans the system SMS provider (content://sms) and imports all messages into SafeInbox.
     * Uses the optimized classifyWithDetails engine for every message.
     * Prevents duplicates via dedup_id checking.
     */
    public void ingestSystemMessages(SpamDetector detector) {
        ...
        try {
            ...
            try {
                ...
                for (com.example.safeinbox.models.ContactModel c : contacts) {
                    String normalized = com.example.safeinbox.utils.ContactUtils.normalizeSender(c.getPhoneNumber());
                    if (normalized.length() >= 10) {
                        contactCache.add(normalized.substring(normalized.length() - 10));
                    }
                }
                detector.setContactCache(contactCache);
            } catch (Exception e) {
                Log.e(TAG, "Contact pre-cache failed", e);
            }
            ...
            List<SmsMessage> batch = new ArrayList<>();
            try {
                ...
                while (cursor.moveToNext()) {
                    ...
                    // Run classification (Fast with cache)
                    ClassificationResult result = detector.classifyWithDetails(sender, body);
                    msg.setClassificationStatus(result.status.name());
                    msg.setHasRiskyLink(result.isRiskyLink);
                    msg.setScore(result.totalScore);
                    
                    msg.setSenderName(ContactUtils.getContactName(context, sender));
                    msg.setDedupId(dedupId);
            ...
            Log.i(TAG, "Ingestion Complete. Imported: " + imported + ", Skipped (Duplicates): " + skipped);
        } catch (Exception e) {
            Log.e(TAG, "System Ingestion Error", e);
        } finally {
            isRescanMode = false;
            detector.setContactCache(null);
            detector.clearSimilarityCache();
        }
    }
    ...
    public void reclassifyProjectWide(SpamDetector detector, RescanProgressCallback callback) {
        try {
            ...
            // 1. Warm the Engine Cache (Crucial for Speed)
            detector.prepareSimilarityCache();
            
            // 2. Pre-fetch Manual Blocklist into memory (O(1) lookup vs O(N) DB hits)
            java.util.Set<String> blocklist = new java.util.HashSet<>(getBlacklistedNumbers());
            java.util.Set<String> archivedDedupIds = getArchivedDedupIds();
            
            // 3. Fetch all messages to process
            ...
            isRescanMode = true; // Turbo: Update cache instead of evicting
                
                // --- TURBO-SPEED: PRE-CACHE CONTACTS (O(1) Memory vs O(N) Provider Hits) ---
                java.util.Set<String> contactCache = new java.util.HashSet<>();
                try {
                    List<com.example.safeinbox.models.ContactModel> contacts = com.example.safeinbox.utils.ContactUtils.getAllContacts(context);
                    for (com.example.safeinbox.models.ContactModel c : contacts) {
                ...
                for (int i = 0; i < total; i++) {
                    ...
                    // INDUSTRIAL THROTTLING: Only report progress every 20 messages to avoid UI jank
                    ...
                    // RUN NEW 6-LEVEL ENGINE (Now with pre-warmed cache)
                    ClassificationResult result = detector.classifyWithDetails(msg.getSender(), msg.getBody());
                    
                    // O(1) Memory Lookup for manual blocks
                    boolean isBlocked = blocklist.contains(normalizeSender(msg.getSender()));
                    // Only definitive SPAM triggers the legacy is_spam=1 flag. 
                    // SUSPICIOUS messages stay in Inbox (is_spam=0).
                    // High-Sensitivity Routing: Both SPAM and SUSPICIOUS are moved to the Vault
                    boolean isThreat = (result.status == ClassificationResult.Status.SPAM || result.status == ClassificationResult.Status.SUSPICIOUS);
                    boolean finalSpam = isBlocked || isThreat;
                    ...
                    v.put(Constants.COL_CLASSIFICATION_STATUS, result.status.name());
                    v.put(Constants.COL_HAS_RISKY_LINK, result.isRiskyLink ? 1 : 0);
                    v.put(Constants.COL_SCORE, result.totalScore);
                    v.put(Constants.COL_REASON, result.reason);
                ...
                db.endTransaction();
                isRescanMode = false; // Reset to real-time mode
                detector.setContactCache(null); // Clear forensic cache
                detector.clearSimilarityCache(); // Free memory
            }
        } catch (Exception e) {
