...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onConfigure(SQLiteDatabase db) {
        ...
        try {
            ...
            // Boost Display: Memory-Mapped I/O for lightning fast reads
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String createMessages = "CREATE TABLE " + Constants.TABLE_MESSAGES + " ("
                ...
                + Constants.COL_REASON + " TEXT, "
                + "UNIQUE(" + Constants.COL_DEDUP_ID + "))";
        db.execSQL(createMessages);

        String createSimilarity = "CREATE TABLE " + Constants.TABLE_SIMILARITY_DATA + " ("
                + Constants.COL_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, "
                + Constants.COL_BODY_HASH + " TEXT UNIQUE, "
                + Constants.COL_BODY + " TEXT, "
                + Constants.COL_LABEL + " TEXT)";
        db.execSQL(createSimilarity);
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (oldVersion < 13) {
            // v13: Add classification_reason and similarity_data table
            try {
                db.execSQL("ALTER TABLE " + Constants.TABLE_MESSAGES + " ADD COLUMN " + Constants.COL_REASON + " TEXT");
                db.execSQL("CREATE TABLE IF NOT EXISTS " + Constants.TABLE_SIMILARITY_DATA + " ("
                        + Constants.COL_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, "
                        + Constants.COL_BODY_HASH + " TEXT UNIQUE, "
                        + Constants.COL_BODY + " TEXT, "
                        + Constants.COL_LABEL + " TEXT)");
                Log.d(TAG, "v13 migration: reason column and similarity table added");
            } catch (Exception e) {
                Log.e(TAG, "v13 migration failed", e);
            }
        }
    }
